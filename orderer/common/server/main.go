...
func Start(cmd string, conf *localconfig.TopLevel) {
	...
	serverConfig := initializeServerConfig(conf) //从orderer中读取配置文件的信息
	...
	service := NewServer(manager, signer, &conf.Debug, conf.General.Authentication.TimeWindow, mutualTLS)

	switch cmd {
	case start.FullCommand(): // "start" command
		logger.Infof("Starting %s", metadata.GetVersionInfo())
		initializeProfilingService(conf)
		ab.RegisterAtomicBroadcastServer(grpcServer.Server(), service)
		logger.Info("Beginning to serve requests")
		grpcServer.Start()
	case benchmark.FullCommand(): // "benchmark" command
		logger.Info("Starting orderer in benchmark mode")
		benchmarkServer := performance.GetBenchmarkServer()
		benchmarkServer.RegisterService(service)
...
func initializeServerConfig(conf *localconfig.TopLevel) comm.ServerConfig {
	...
	// check to see if TLS is enabled  进行安全检查 这里不需要
	if secureOpts.UseTLS {
		msg := "TLS"
		// load crypto material from files
		println(conf.General.TLS.Certificate)
...
func initializeBootstrapChannel(conf *localconfig.TopLevel, lf blockledger.Factory) {
	...
	println("here...", conf.General.GenesisFile)
	println(conf.General.GenesisMethod)
	// Select the bootstrapping mechanism
	//初始的GenesisBlock Number 为0
	switch conf.General.GenesisMethod {
	case "provisional":
		println("SystemChannel", conf.General.SystemChannel)
		genesisBlock = encoder.New(genesisconfig.Load(conf.General.GenesisProfile)).
			GenesisBlockForChannel(conf.General.SystemChannel)
	case "file":
	...
	println("获取对账本的执行对象，然后将block写入文件中")
...
func initializeLocalMsp(conf *localconfig.TopLevel) {
	...
	logger.Infof("localMSPID", conf.General.LocalMSPID)
}
...
func initializeMultichannelRegistrar(conf *localconfig.TopLevel, signer crypto.LocalSigner,
	...
	println("chainIds", len(lf.ChainIDs()))
	if len(lf.ChainIDs()) == 0 {
		println("bootstrap 初始化")
