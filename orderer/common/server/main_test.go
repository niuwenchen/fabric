...
func TestInitializeLoggingLevel(t *testing.T) {
	initializeLoggingLevel(
		&localconfig.TopLevel{
			...
			General: localconfig.General{LogLevel: "foo=Info"},
		},
	)
	assert.Equal(t, flogging.GetModuleLevel("foo"), "INFO")
}

func TestInitializeProfilingService(t *testing.T) {
	// get a free random port
	listenAddr := func() string {
		l, _ := net.Listen("tcp", "localhost:0") // 启动一个server
		l.Close()
...
func TestInitializeLocalMsp(t *testing.T) {
	t.Run("Happy", func(t *testing.T) {
		assert.NotPanics(t, func() {
			...
			println("localMSPDir", localMSPDir)
			initializeLocalMsp(
				&localconfig.TopLevel{
					General: localconfig.General{
