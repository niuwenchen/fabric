...
func serve(args []string) error {
	...
	// the peer only comes up with the standard MSP    peer只是来自于标准的MSP格式文件
	mspType := mgmt.GetLocalMSP().GetType()
	println("msp FABRIC", msp.FABRIC)
	println(msp.ProviderTypeToString(mspType))
	...
	println("listenAddr", listenAddr)

	serverConfig, err := peer.GetServerConfig()
	// ServerConfig 的获取信息 和orderer的信息是感觉是一样的

	if err != nil {
		logger.Fatalf("Error loading secure config for peer (%s)", err)
	}

	fmt.Println(serverConfig.SecOpts.UseTLS)
	...
	println("signingIdentity", signingIdentity.GetMSPIdentifier()) //OrgsMSP
	...
	peer.Initialize(func(cid string) {
		logger.Infof("Deploying system CC, for channel <%s>", cid)
...
func createChaincodeServer(ca tlsgen.CA, peerHostname string) (srv *comm.GRPCServer, ccEndpoint string, err error) {
	...
	println("peerHostname", peerHostname, ccEndpoint)
	if err != nil {
	...
	println("cclistenAddress", cclistenAddress)
	if cclistenAddress == "" {
...
func registerChaincodeSupport(grpcServer *comm.GRPCServer, ccEndpoint string, ca tlsgen.CA, packageProvider *persistence.PackageProvider, aclProvider aclmgmt.ACLProvider, pr *platforms.Registry) (*chaincode.ChaincodeSupport, ccprovider.ChaincodeProvider, *scc.Provider) {
	...
	lsccInst := lscc.New(sccp, aclProvider, pr)                            //	lsccInst := lscc.New(sccp, aclProvider, pr) //
	lifecycleSCC := &lifecycle.SCC{}

	println("registerChaincodeSupport....")
...
func startChaincodeServer(peerHost string, aclProvider aclmgmt.ACLProvider, pr *platforms.Registry) (*chaincode.ChaincodeSupport, ccprovider.ChaincodeProvider, *scc.Provider, *persistence.PackageProvider) {
	...
	println("chaincodeInstallPath", chaincodeInstallPath)
...
func startAdminServer(peerListenAddr string, peerServer *grpc.Server) {
	...
	println("admin Server:", peerListenAddr, adminListenAddress)
	...
	println("adminPolicy...")
	println("bool....", separateLsnrForAdmin)
	...
	fmt.Println("admin server ending ")
