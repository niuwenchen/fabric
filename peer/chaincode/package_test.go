...
func TestCDSPackage(t *testing.T) {
	...
	// peer chaincode package -n somecc -p "somg/go/package" -v 0 ccpack.file
	...
	println("创建CS，将CS包装到CDS，然后存储到文件")
	b, err := ioutil.ReadFile(ccpackfile)
	println("read ccpackfile")
	if err != nil {
...
func TestSignedCDSPackage(t *testing.T) {
	...
	e := &pcommon.Envelope{}
	err = proto.Unmarshal(b, e) // 将 cds 封装到信封中
	println("unmarshal")
	println(e)
	if err != nil {
