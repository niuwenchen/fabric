...
func getChaincodeInstallPackage(cds *pb.ChaincodeDeploymentSpec, cf *ChaincodeCmdFactory) ([]byte, error) {
	...
	println("signCCDepSpec", signCCDepSpec)
	...
	if ip == "" {
		...
		// 如果没有给定instanitaion 策略，默认: admin 必须签名chaincode 实例申请
		println("local mspid " + mspid)
		if err != nil {
			return nil, err
		}
		ip = "AND('" + mspid + ".admin')"
...
func chaincodePackage(cmd *cobra.Command, args []string, cdsFact ccDepSpecFactory, cf *ChaincodeCmdFactory) error {
	...
	logger.Infof("Packaged chaincode into deployment spec of size <%d>, with args = %v", len(bytesToWrite), args)
	fileToWrite := args[0]
