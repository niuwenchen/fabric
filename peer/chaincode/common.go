...
func getChaincodeSpec(cmd *cobra.Command) (*pb.ChaincodeSpec, error) {
	...
	println("createSignedCCDepSpec", createSignedCCDepSpec)
...
func chaincodeInvokeOrQuery(cmd *cobra.Command, invoke bool, cf *ChaincodeCmdFactory) (err error) {
	println("common chaincodeInvokeOrQueue")
	...
	// 这是主方法，用来调用ChaincodeInvoke和查询
	println("调用 ChaincodeINcokeOrQuery")
	...
	println("ChaincodeInvokeOrQuery ", err != nil, invoke)
	if err != nil {
	...
	if invoke {
		...
		if chaincodeQueryRaw && chaincodeQueryHex {
			fmt.Print("options --raw (-r) and --hex (-x) are not compatible")
...
func checkChaincodeCmdParams(cmd *cobra.Command) error {
	...
	println("cid.Name " + cmd.Name() + " " + escc + "policy " + policy + " collectionsConfigFile " + collectionsConfigFile + "chaincodeName " + chaincodeName)
	...
	println("chaincodeCtorJson", chaincodeCtorJSON)
	if chaincodeCtorJSON != "{}" {
		...
		println("argsPresent", argsPresent, "  funcPresent", funcPresent)
...
func validatePeerConnectionParameters(cmdName string) error {
	println("validatePeerConnectionParameters  ", cmdName, "-->", connectionProfile, common.UndefinedParamValue, connectionProfile != common.UndefinedParamValue, len(peerAddresses), len(tlsRootCertFiles), viper.GetBool("peer.tls.enabled"))
	println(peerAddresses[0])
...
func ChaincodeInvokeOrQuery(
	...
	// Build the ChaincodeInvocationSpec message 构造cis
	...
	println("创建ChaincodeProposal 在invoke中进行调用")
	...
	if invoke {
		if proposalResp != nil {
			...
			// assemble a signed transaction (it's an Envelope message)  组装一个标志的交易
			...
			// send the envelope for ordering  发送给orderer
