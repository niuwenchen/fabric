...
admin
	实现对服务器的控制: 获取状态 ，启动服务器（这两个函数的内部实现都是一样的）
	对日志的控制。
...
func (s *ServerAdmin) GetStatus(ctx context.Context, env *common.Envelope) (*pb.ServerStatus, error) {
	...
	//	return nil, err
	//}
	_, err := s.v.validate(ctx, env)
	if err != nil {
		return nil, err // 这种验证方法，如果validate返回的参数是error，就代表返回失败，err不是nil，而是error对象，因为err真的不是nil‘
	}
	status := &pb.ServerStatus{Status: pb.ServerStatus_STARTED}
	logger.Infof("returning status: %s", status)
	return status, nil
}
