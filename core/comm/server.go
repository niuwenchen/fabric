...
func NewGRPCServerFromListener(listener net.Listener, serverConfig ServerConfig) (*GRPCServer, error) {
	...
	serverOpts = append(serverOpts, grpc.MaxSendMsgSize(MaxSendMsgSize)) //设置发送消息的最大长度
	serverOpts = append(serverOpts, grpc.MaxRecvMsgSize(MaxRecvMsgSize)) //设置
...
func (gServer *GRPCServer) Start() error {
	return gServer.server.Serve(gServer.listener) // 启动这个服务
}
