...
func TestEndorserNilProp(t *testing.T) {
	es := endorser.NewEndorserServer(pvtEmptyDistributor, &em.MockSupport{
		...
		//ExecuteResp 在support中没有，这里构造了一个
...
//simulate--->3 callChaincode--->Execute 的时候调用mock的数据
...
//测试Chaincode的具体信息 ChaincodeData
...
//测试背书服务系统chaincode
// 这里测试通过，但是这里面的mock 和 attachPluginEndorser 函数不确定
// 注释之后测试失败
func TestEndorserSysCC(t *testing.T) {
	...
	attachPluginEndorser(support) // 这个方法是什么意思
...
//LSCC  测试chaincode 本地chaincode的错误类型
...
func TestEndorserDupTXId(t *testing.T) {
	...
	//fmt.Println("GetTransactionByIDErr")
	signedProp := getSignedProp("ccid", "0", t)
...
//Channel  signedProposal: 无效的channelID
