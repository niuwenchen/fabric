...
func (r *Registry) Register(ccid *ccintf.CCID, cc shim.Chaincode) error {
	name := ccid.GetName()
	inprocLogger.Infof("Registering chaincode instance: %s", name)
...
func (vm *InprocVM) getInstance(ipctemplate *inprocContainer, instName string, args []string, env []string) (*inprocContainer, error) {
	...
	inprocLogger.Infof("chaincode instance created for %s", instName)
	return ipc, nil
}
...
func (ipc *inprocContainer) launchInProc(id string, args []string, env []string) error {
	...
	go func() {
		defer close(ccchan)
		inprocLogger.Infof("chaincode started for %s", id)
		if args == nil {
			args = ipc.args
		}
		if env == nil {
		...
		println("在这里调用shimStartInProc")
		err := shimStartInProc(env, args, ipc.chaincode, ccRcvPeerSend, peerRcvCCSend)
		if err != nil {
			err = fmt.Errorf("chaincode-support ended with err: %s", err)
			_inprocLoggerErrorf("%s", err)
		}
		inprocLogger.Infof("chaincode ended for %s with err: %s", id, err)
	}()
	...
	case <-ccchan:
		close(peerRcvCCSend)
		println("inproccontroller.go 中不清楚的地方")
